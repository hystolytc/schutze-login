...
const Login = () => {
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (isIncorrect) setIsIncorrect(!isIncorrect)
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!(usernameValidator(username) && passwordValidator(password)))
      return setIsIncorrect(!isIncorrect)
  ...
  return (
    <div className='relative flex justify-center items-center w-screen h-screen bg-primary'>
      <img 
        className='absolute top-4 left-4 w-80 object-contain'
        src={logo}
        alt='company logo' />

      <section className='px-20 py-16 rounded-lg w-[480px] bg-white'> 
